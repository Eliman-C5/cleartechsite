...
const TestimonialCard = ({ img, job, desc }) => {
  return (
    <div
      className=" py-6 lg:py-0 mb-5 lg:w-[365px] lg:h-[340px] flex flex-col items-center justify-center"
      style={{
    ...
    >
      <div className="w-[90%] h-[90%]">
        ...
        <p className="roboto text-base font-normal text-[#999999] my-2">{desc}</p>
        <div className="flex items-center">
          {/* <p className=" text-color02_blue font-bold ">{name}</p> */}
          <img src={img} alt={img} className="w-[60px] h-[60px] rounded-full mr-4" />
          <p className="roboto text-base font-normal text-[#999999]">{job}</p>
        </div>
      </div>
    </div>
  );
